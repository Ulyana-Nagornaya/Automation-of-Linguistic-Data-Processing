...
class UDPipeAnalyzer(LibraryWrapper):
    ...
    def _bootstrap(self) -> AbstractCoNLLUAnalyzer:
        ...
        model.add_pipe(
            "conll_formatter",
            last=True,
            config={"conversion_maps": {"XPOS": {"": "_"}}, "include_headers": False},
        )
...
class POSFrequencyPipeline:
    ...
    def summarise_pos(self) -> dict[str, int]:
        """
        Compute all pos from all files
        """

        pos_freq = {}
        for _, article in self._corpus_manager.get_articles().items():
            for connllu_sentence in self._analyzer.from_conllu(article).sentences:
                ...
                for word in set(words):
                    if word in pos_freq:
                        pos_freq[word] += words.count(word)
                    else:
                        pos_freq[word] = 1
